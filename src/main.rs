...
fn create_block(previous_index: u32, logt: String, log_data: String, previous_hash: Vec<u8>, salt: String)->Block{
    Block{
...
fn load_blockchain() -> Vec<Block>{
    let file = fs::File::open("/home/abi/block_log/blockchain.json").expect("File not found");
    let deserialized_json: Vec<Block> = serde_json::from_reader(file).expect("Error while reading file");
    println!("Blockchain Loaded");
    deserialized_json
}

fn save_blockchain( chain: &Vec<Block>){
    serde_json::to_writer(&fs::File::create("/home/abi/block_log/blockchain.json").unwrap(), &chain).expect("error");
...
fn log_parser()-> Vec<Block>{
    ...
    let iter_logs = read_lines("/home/abi/block_log/logs/bash_log.txt").unwrap();
    for line in iter_logs{
        ...
        chain.push(create_block(*previous_index, logt, data, previous_hash, String::from(&salt)));
